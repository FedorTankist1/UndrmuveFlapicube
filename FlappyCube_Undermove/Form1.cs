...
namespace FlappyCube_Undermove
{
    public partial class Form1 : Form
    {
        ...
        private void SettingsUbdateTimer_Tick(object sender, EventArgs e)
        {
            try
            {
                string[] Setttings;
                Setttings = File.ReadAllLines("settings");
                if (Setttings.Length == 3)
                {
                    PlayerTimer.Interval = Convert.ToInt32(Setttings[0]);
                    TubesTimer.Interval = Convert.ToInt32(Setttings[0]);
                    isGloingon = Convert.ToBoolean(Setttings[2]);
                }
            }
